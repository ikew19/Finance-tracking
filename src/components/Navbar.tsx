...
import { ArrowLeftRight, BadgeDollarSign, HandCoins, LayoutDashboard, User, Wallet } from "lucide-react";
      ...
      <footer className="min-h-24 text-neutral-100 antialiased ml-3"> 
        <div className="flex items-center group gap-3 text-left">
          <div type={() => setIsOpen(!isOpen)}
            ...
            <User/>
            {isOpen && 
            <div className="bg-white rounded-2xl shadow-lg  bottom-16 p-4">
              <button className="text-lg font-semibold text-black">Log out</button>
            </div>}
          </div>

          <div
          className="group-hover:block sm:block bg-emerald-600 sm:bg-transparent p-1 rounded-xl">
